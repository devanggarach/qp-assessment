...
export class UserRepository {
	...
	static async createUser(traceId: string, data: any) {
		try {
			return await prisma.user.create({ data });
		} catch (error) {
			throw this.serviceResponse.databaseError({
				traceId,
				errorCode: ERROR.DB_CREATE_USER.CODE,
				message: ERROR.DB_CREATE_USER.MESSAGE,
				error: ERROR.DB_CREATE_USER.ERROR,
				meta: error,
			});
		}
	}
